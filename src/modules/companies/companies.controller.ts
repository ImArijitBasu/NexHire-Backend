...
export const companiesController = {
  async getAll(req: Request, res: Response) {
    try {
      const result = await companiesService.getAll(req.query as Record<string, string>);
      res.json({ success: true, ...result });
    } catch (error: any) { res.status(500).json({ success: false, error: 'Failed to fetch companies' }); }
  },
  async getBySlug(req: Request, res: Response) {
    try {
      const company = await companiesService.getBySlug(req.params.slug as string);
  ...
  async update(req: AuthRequest, res: Response) {
    try {
      const company = await companiesService.update(req.params.id as string, req.body, req.user!.id, req.user!.role);
      res.json({ success: true, company });
    } catch (error: any) { res.status(error.status || 500).json({ success: false, error: error.message || 'Failed' }); }
  },
